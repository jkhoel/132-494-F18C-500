...
\cSection{INTERIOR CHECKS}
  \cSubSection{Left Console}
    \cItem{OBOGS Control Sw. and Flow Knob}{OFF}
    \cItem{Circuit Breakers - 4 pcs}{PUSHED IN}
    \cItem{Mission Computer}{NORM}
    \cItem{Hydraulic Isolation Switches}{NORM}
    \cItem{COMM 1/IFF Antennae}{AUTO/BOTH}
    \cItem{Communications Panel}{SET}
    \cItem{Volume Panel}{SET}
    \cItem{GEN TIE}{NORM - GUARD DOWN}
    \cItem{Gain Switch}{NORM}
    \cItem{Refuel Probe Switch}{RETRACT}
    \cItem{External Tank Switches}{NORM}
    \cItem{Dump Switch}{OFF}
    \cItem{Internal Wing Switch}{NORM}
    \cItem{External Lights}{ACC. TO SOP}
    \cItem{Master Light Switch}{FORWARD}
    \cItem{Throttles}{OFF}
    \cItem{Parking Brake}{PARK}
...
\cSection{BEFORE TAXI}
  ...
  \cItem{Wingfold}{SPREAD AND LOCK}
...
\cSection{If still no reset}
  \cItem{FCS Circuit Breakers - 4pcs}{PULL}
  \cItem{Wait 10 seconds}{CHECK}
  \cItem{FCS Circuit Breakers}{RESET}
  \cItem{FCS RESET button}{PUSH}

\cSection{Reset ok}
  \cItem{Flaps}{AUTO}
  \cItem{FCS RESET and Paddle Switch}{ACTUATE}
  \cItem{Flaps}{HALF}
  \cItem{FCS Initiated BIT}{PERFORM}
  \cComment{If BLIN codes other than 246, 270, 344, 375, 574, 604, 5005, 5032 remain following IBIT, aircraft requires maintainance.\\}
  \cItem{Trim}{CHECK}
  \cItem{T.O. trim button}{PUSH until TRIM ADVISORY}
  \cItem{Flaps}{AUTO}

\cSection{Controls Check}
  \cItem{Control Stick}{CYCLE}
  \cItem{Flaps}{HALF}
  \cItem{Rudder Pedals}{CYCLE}
  \cItem{Trim}{SET}
  \cComment{Shore - 12°\\ CV - 44,000 lbs and below - 16°\\ CV - 45,000-48,000 lbs - 17°\\ CV - 49,000 lbs and above - 19°\\}
  \cItem{Probe/Brakes/Launch Bar/Hook}{CYCLE}
...
\cSection{Continue}
  ...
  \cItem{Radar Altimeter}{SET}
  \cComment{Shore - 450 ft\\ CV - 375 ft\\}
